...
namespace Game.Player
{
    public class CharacterPhysics : MonoBehaviour
    {
        ...
        [SerializeField][Range(-10, 10)] private float groundCheckDistance = 0.1f;
        ...
        {
            Vector3 offsetVector = transform.position;
            Vector3[] GroundCheckOffsets = GetGroundCheckOffsets();
            offsetVector.y += GroundCheckOffset;
            int arrayLength = GroundCheckOffsets.Length;
            for (int i = 0; i < arrayLength; ++i)
                Debug.DrawRay(offsetVector + GroundCheckOffsets[i], -Vector3.up * GroundCheckDistance);
        }
